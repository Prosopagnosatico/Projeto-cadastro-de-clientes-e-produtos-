...
public class AlteracaoCliente extends JFrame {
    ...
    private JRadioButton generoFemButton, generoOButton;
    ...
    public AlteracaoCliente() {
        ...
        gbc.weightx = 0.0;
        mainPanel.add(new JLabel("Idade"), gbc);

        idadeField = new JTextField(10);
        gbc.gridx = 3; gbc.gridy = 1;
        mainPanel.add(idadeField, gbc);
    ...
    private void addListeners() {
        salvarButton.addActionListener(e -> {
            ...
            cliente.setEstado2((String) estadoComboBox.getSelectedItem());
            cliente.setRua(ruaField.getText());
            cliente.setCidade(cidadeField.getText());
            cliente.setNivel((String) nivelComboBox.getSelectedItem());
            cliente.setStatus((String) statusComboBox.getSelectedItem());
            cliente.setIdentificador(cpfField.getText());

            String generos = "";
            if (generoMascButton.isSelected()) {
                generos = "Masculino";
                cliente.setGen(generos);
            } else if (generoFemButton.isSelected()) {
                generos = "Feminino";
                cliente.setGen(generos);
            } else if (generoOButton.isSelected()) {
                generos = "Outros";
                cliente.setGen(generos);
            }

            GerenciaCliente gerenciaCliente = new GerenciaCliente();
            gerenciaCliente.atualizarCliente(cliente);
